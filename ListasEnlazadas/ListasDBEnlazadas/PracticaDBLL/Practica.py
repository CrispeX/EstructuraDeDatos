...
def eliminar_camiones(avenida):
    ...
    while actual is not None:
        ...
        previo = actual.prev
        if (actual.tipo.lower() == "camion" and
            actual.prioridad > 3 and
            previo is not None and
            siguiente is not None and
            previo.tipo.lower() == "auto" and
            siguiente.tipo.lower() == "auto"):
            if previo is not None:
                previo.next = siguiente
            else:
                avenida.head = siguiente
...
def bloque_critico(avenida):
    curr = avenida.head
    while curr:
        prev = curr.prev
        nxt = curr.next
        if prev is not None and nxt is not None:
            if (prev.tipo.lower() == curr.tipo.lower() == nxt.tipo.lower() and
                curr.prioridad < prev.prioridad and
                curr.prioridad > nxt.prioridad):
                return True
        curr = curr.next
    return False
...
avenida2 = Via()
avenida2.append(Vehiculo("AAA111", "auto", 4))
avenida2.append(Vehiculo("BBB222", "auto", 3))
avenida2.append(Vehiculo("CCC333", "auto", 2))
print(f"\nPrueba bloque critico en: {avenida2}")
print(f"¿Hay bloque critico?: {bloque_critico(avenida2)}")
